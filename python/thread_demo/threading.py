...
# 返回一个新的锁对象。初始情况下锁处于解锁状态。
_allocate_lock = _thread.allocate_lock
_set_sentinel = _thread._set_sentinel
# 返回当前线程的 “线程标识符”。它是一个非零的整数。线程标识符可能会在线程退出，新线程创建时被复用。
...
def setprofile(func):
    """为所有从threading模块启动的线程设置profile函数
...
class _RLock:
    ...
    def acquire(self, blocking=True, timeout=-1):
        ...
        当无参数调用时： 如果这个线程已经拥有锁，递归级别增加一，并立即返回。否则，如果其他线程拥有该锁，则阻塞至该锁解锁。
        一旦锁被解锁(不属于任何线程)，则抢夺所有权，设置递归等级为一，并返回。
        如果多个线程被阻塞，等待锁被解锁，一次只有一个线程能抢到锁的所有权。在这种情况下，没有返回值。
    ...
    def release(self):
        ...
        释放锁，自减递归等级。
        如果减到零，则将锁重置为非锁定状态(不被任何线程拥有)，并且，如果其他线程正被阻塞着等待锁被解锁，则仅允许其中一个线程继续。
